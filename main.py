...
# Helper functions to get current API instances with latest settings
def get_current_tautulli_api():
    """Get a Tautulli API client with current settings from database."""
    base_url = db.get_setting('tautulli.base_url', 'http://localhost:8181')
    api_key = db.get_setting('tautulli.api_key', '')
    return TautulliAPI(base_url, api_key)

def get_current_history_processor():
    """Get a HistoryProcessor with current settings from database."""
    api = get_current_tautulli_api()
    return HistoryProcessor(api, db, app.config)
...
def unmatched():
    ...
    # Handle 'all' option - set to a very large number
    if per_page_param == 'all':
        per_page = 999999
    else:
        try:
            per_page = max(1, int(per_page_param))  # Ensure at least 1
    ...
    total_pages = (total + per_page - 1) // per_page if per_page < 999999 else 1
    has_prev = page > 1
...
def api_scan():
    ...
    try:
        ...
        # Get history processor with current settings
        temp_processor = get_current_history_processor()
...
@app.route('/api/get-unmatched-ids', methods=['GET'])
def api_get_unmatched_ids():
    """API endpoint to get list of unmatched item IDs."""
    try:
        ...
        logging.exception("Error getting unmatched IDs")
        return jsonify({'success': False, 'message': str(e)})

@app.route('/api/get-ignored-ids', methods=['GET'])
def api_get_ignored_ids():
    """API endpoint to get list of ignored item IDs."""
    try:
        items, total = db.get_ignored_items(page=1, per_page=999999)
        item_ids = [item['id'] for item in items]
        return jsonify({
            'success': True,
            'item_ids': item_ids
        ...
        logging.exception("Error getting ignored IDs")
        return jsonify({'success': False, 'message': str(e)})
...
def api_fix_all():
    ...
    try:
        ...
        # Attempt to fix all items with current settings
        processor = get_current_history_processor()
        fixed_count = processor.fix_all_unmatched()
...
@app.route('/api/fix-all-ignored', methods=['POST'])
def api_fix_all_ignored():
    """API endpoint to attempt to fix all ignored items."""
    try:
        # Get all ignored items (not fixed)
        ignored_items, total = db.get_ignored_items(page=1, per_page=999999)
        
        if not ignored_items:
            return jsonify({
                'success': True,
                'message': 'No ignored items to fix.'
            })
        
        # Attempt to fix each ignored item
        processor = get_current_history_processor()
        fixed_count = 0
        
        for item in ignored_items:
            item_id = item['id']
            result = processor.fix_unmatched_item(item_id)
            if result:
                fixed_count += 1
        
        return jsonify({
            'success': True,
            'message': f"Fix attempt completed. Fixed {fixed_count} of {len(ignored_items)} ignored items."
        })
    except Exception as e:
        logging.exception("Error during fix all ignored")
        return jsonify({'success': False, 'message': str(e)})
...
def api_fix_item(item_id):
    ...
    try:
        # Use current settings for fix attempt
        processor = get_current_history_processor()
        result = processor.fix_unmatched_item(item_id)
...
def api_manual_fix_search():
    ...
    try:
        ...
        api = get_current_tautulli_api()
        results = api.search_title(search_query, None, media_type, history_year=None, manual_search=True)
...
def api_manual_fix_update():
    ...
    try:
        ...
        if tautulli_api_key:
            try:
                tautulli = get_current_tautulli_api()
...
if __name__ == '__main__':
    ...
    # Debug mode should only be enabled in development via FLASK_DEBUG=true environment variable
    # For security, debug mode is disabled by default to prevent code execution and information disclosure
    debug_mode = os.environ.get('FLASK_DEBUG', 'False').lower() == 'true'
    app.run(debug=debug_mode, host='0.0.0.0', port=6120)
